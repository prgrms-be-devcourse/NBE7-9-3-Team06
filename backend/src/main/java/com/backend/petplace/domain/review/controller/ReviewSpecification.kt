...
interface ReviewSpecification {
    @Operation(
        summary = "리뷰 이미지 Presigned URL 생성",
        description = "S3에 직접 이미지를 업로드할 수 있는 Presigned URL을 생성합니다."
    )
    fun getPresignedUrl(
        @Parameter(description = "업로드할 원본 파일명", required = true) filename: String
    ): ResponseEntity<ApiResponse<PresignedUrlResponse?>>

    @ApiErrorCodeExamples(ErrorCode.NOT_FOUND_MEMBER, ErrorCode.NOT_FOUND_PLACE)
    @Operation(
        summary = "리뷰 등록",
        description = "특정 장소에 대한 리뷰를 등록합니다. 내용, 별점은 필수이며 이미지는 선택입니다. ✅ Presigned URL로 이미지 업로드 후, 리뷰 정보를 최종 저장합니다."
    )
    fun createReview(
        @Parameter(
            description = "리뷰 정보 - 장소ID, 내용, 별점, S3 이미지 경로",
            required = true
    ...
    fun getReviewsByPlace(
        @Parameter(`in` = ParameterIn.PATH, description = "장소 ID", required = true) placeId: Long
    ...
    fun getMyReviews(userDetails: CustomUserDetails?): ResponseEntity<ApiResponse<List<MyReviewResponse>?>>
}